...
### Mock 模式
系统默认配置为 `LLM_MODE=mock`，无需配置 API Key，适合本地快速演示和流程测试。
...
1. **Perception Agent**：负责告警标准化、去重、聚合。
2. **Diagnosis Agent**：调用指标、日志、拓扑、Runbook 等工具进行结构化推理（ReAct）。
3. **Remediation Agent**：推荐处置动作，并生成执行脚本草案。
4. **Postmortem Agent**：在故障恢复后，自动生成结构化 RCA 报告。
